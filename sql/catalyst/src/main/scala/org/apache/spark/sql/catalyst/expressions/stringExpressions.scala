...
trait TrimExpression extends Expression with ImplicitCastInputTypes {
  ...
  protected def trimOpt: Option[Expression]
  protected def direction: String

  override def children: Seq[Expression] = srcExpr +: trimOpt.toSeq
  ...
  protected def doEval(input: InternalRow, srcArr: Array[Byte]): Any
  ...
  private lazy val evalFunc = srcExpr.dataType match {
    ...
    case BinaryType =>
      (input: InternalRow) => {
        val srcArr = srcExpr.eval (input).asInstanceOf[Array[Byte]]
        if (srcArr == null) {
          null
        } else {
          doEval(input, srcArr)
  ...
  private lazy val genCodeFunc: (CodegenContext, ExprCode) => ExprCode = srcExpr.dataType match {
    case StringType =>
      (ctx, ev) => {
        val evals = children.map(_.genCode(ctx))
          ...
          ev.copy(evals.map(_.code) :+
            code"""
             ...
             if (${srcArr.isNull}) {
               ...
               ${ev.value} = org.apache.spark.unsafe.types.ByteArray.$trimMethod(${srcArr.value});
             }""")
        } else {
          val trimArr = evals(1)
          val getTrimFunction =
            s"""
             if (${trimArr.isNull}) {
               ...
               ${ev.value} = org.apache.spark.unsafe.types.ByteArray
                 .$trimMethod(${srcArr.value}, ${trimArr.value});
             }"""
          ev.copy(evals.map(_.code) :+
            code"""
             boolean ${ev.isNull} = false;
  ...
  override def sql: String = if (trimOpt.isDefined) {
    s"TRIM($direction ${trimOpt.get.sql} FROM ${srcExpr.sql})"
  } else {
    super.sql
  }
}
...
case class Trim(
    srcExpr: Expression,
    trimOpt: Option[Expression] = None)
  ...
  override def doEval(input: InternalRow, srcString: UTF8String): Any = {
    if (trimOpt.isDefined) {
      srcString.trim(trimOpt.get.eval(input).asInstanceOf[UTF8String])
    } else {
      srcString.trim()
    }
  }

  override def doEval(input: InternalRow, srcArr: Array[Byte]): Any = {
    if (trimOpt.isDefined) {
      ByteArray.trim(srcArr, trimOpt.get.eval(input).asInstanceOf[Array[Byte]])
    } else {
      ByteArray.trim(srcArr)
    }
  }
...
case class TrimLeft(
  ...
  override def doEval(input: InternalRow, srcString: UTF8String): Any = {
    if (trimOpt.isDefined) {
      srcString.trimLeft(trimOpt.get.eval(input).asInstanceOf[UTF8String])
    } else {
      srcString.trimLeft()
    }
  }
  ...
  override def doEval(input: InternalRow, srcArr: Array[Byte]): Any = {
    if (trimOpt.isDefined) {
      ByteArray.trimLeft(srcArr, trimOpt.get.eval(input).asInstanceOf[Array[Byte]])
    } else {
      ByteArray.trimLeft(srcArr)
    }
  }
...
case class TrimRight(
  ...
  override def doEval(input: InternalRow, srcString: UTF8String): Any = {
    if (trimOpt.isDefined) {
      srcString.trimRight(trimOpt.get.eval(input).asInstanceOf[UTF8String])
    } else {
      srcString.trimRight()
    }
  }
  ...
  override def doEval(input: InternalRow, srcArr: Array[Byte]): Any = {
    if (trimOpt.isDefined) {
      ByteArray.trimRight(srcArr, trimOpt.get.eval(input).asInstanceOf[Array[Byte]])
    } else {
      ByteArray.trimRight(srcArr)
    }
  }
