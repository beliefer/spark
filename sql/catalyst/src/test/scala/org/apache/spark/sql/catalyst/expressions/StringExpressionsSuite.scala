...
class StringExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("Check TrimExpression.checkInputDataTypes results") {
    assert(new Trim(Literal(" aa  ")).checkInputDataTypes().isSuccess)
    assert(new Trim("aa", "a").checkInputDataTypes().isSuccess)
    assert(new Trim(Literal(" aabbtrimccc"), Literal("ab cd")).checkInputDataTypes().isSuccess)
    assert(new TrimLeft(Literal(" aa  ")).checkInputDataTypes().isSuccess)
    assert(new TrimLeft(" aa  ", "a").checkInputDataTypes().isSuccess)
    assert(new TrimLeft(Literal(" aabbtrimccc"), Literal("ab cd")).checkInputDataTypes().isSuccess)
    assert(new TrimRight(Literal(" aa  ")).checkInputDataTypes().isSuccess)
    assert(new TrimRight(" aa  ", "a").checkInputDataTypes().isSuccess)
    assert(new TrimRight(Literal(" aabbtrimccc"), Literal("ab cd")).checkInputDataTypes().isSuccess)

    assert(new Trim(Literal(" aa  ".getBytes)).checkInputDataTypes().isSuccess)
    assert(new Trim("aa".getBytes, "a".getBytes).checkInputDataTypes().isSuccess)
    assert(new Trim(Literal(" aabbtrimccc".getBytes), Literal("ab cd".getBytes)).
      checkInputDataTypes().isSuccess)
    assert(new TrimLeft(Literal(" aa  ".getBytes)).checkInputDataTypes().isSuccess)
    assert(new TrimLeft(" aa  ".getBytes, "a".getBytes).checkInputDataTypes().isSuccess)
    assert(new TrimLeft(Literal(" aabbtrimccc".getBytes), Literal("ab cd".getBytes)).
      checkInputDataTypes().isSuccess)
    assert(new TrimRight(Literal(" aa  ".getBytes)).checkInputDataTypes().isSuccess)
    assert(new TrimRight(" aa  ".getBytes, "a".getBytes).checkInputDataTypes().isSuccess)
    assert(new TrimRight(Literal(" aabbtrimccc".getBytes), Literal("ab cd".getBytes)).
      checkInputDataTypes().isSuccess)

    assert(new Trim(1).checkInputDataTypes().isFailure)
    assert(new Trim("aa", "a".getBytes).checkInputDataTypes().isFailure)
    assert(new Trim("aa".getBytes, "a").checkInputDataTypes().isFailure)
  }
