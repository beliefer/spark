...
case class PlanAdaptiveRuntimeFilterFilters(
  ...
  def apply(plan: SparkPlan): SparkPlan = {
    ...
    plan.transformAllExpressionsWithPruning(
      ...
      SubqueryAdaptiveBroadcastExec(name, indices, true, _, buildKeys,
              ...
              case p @ ProjectExec(_, inputAdapter: InputAdapter)
                ...
                val exchangeProxy = exchange match {
                  case e: BroadcastExchangeExec =>
                    val broadcastValues = SubqueryBroadcastExec(name, indices, buildKeys, e)
                    val newOutput = filterCreationSidePlan.output
                      .filter(_.semanticEquals(buildKeys(indices.head)))
